...
///////////////////////////////////////////////////////////////////////////////////////////////////////
#define PROGRAMBLACKLIST                "Inject", "x1nject", "InjeX", "AC Tool", "Trainer", "Train3r", "AutoIt v3", "AutoIt", "Speed", "Injector", "Process Explorer", "Hack", "Cheat Engine", "Memory View", "Process List", "Speed"
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
void HACKING(){
	...
	while(1){
	  ...
	  fopen_s(&logs, "UnderLogs.txt","a");
	  fputs("Anti-Hack Underclock Detectado!\n",logs);
	  fclose(logs);
	  ExitProcess(0);
	  fopen_s(&logs, "UnderLogs.txt","a");
	  fputs("Anti-Hack DLL Hack Detectado!\n",logs);
	  fclose(logs);
	  ExitProcess(0);
	  }else if(ProgramDetected == true){
	  fopen_s(&logs, "UnderLogs.txt","a");
	  fputs("Anti-Hack 3RD Party Hack Detectado!\n",logs);
	  fclose(logs);
	  ExitProcess(0);
	  }
	Sleep(100);
...
//MD5 checker
void MD5(){
	 if(_stricmp(md5.getHashFromFile("system.mrs").c_str(), "7946102c71d35198a76742d61be8b54b")){
	  fopen_s(&logs, "UnderLogs.txt","a");
	  fputs("System.mrs errada!!\n",logs);
	  fclose(logs);
		ExitProcess(0);
	}
}
