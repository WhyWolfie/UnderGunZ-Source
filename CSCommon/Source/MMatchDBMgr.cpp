...
// @AID int
TCHAR g_szUpdateAccountLastLogoutTime[] = _T( "{CALL spUpdateAccountLastLogoutTime (%u, %u, %u)}" );
...
bool MMatchDBMgr::GetAccountInfo( const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo, const int nServerID )
{
	...
	poutAccountInfo->m_dwLastVIPAnnounce = (unsigned long)rs.Field("LastVIPWall").AsInt();
	poutAccountInfo->m_nVIPWalls	     = rs.Field("VIPWalls").AsInt();
...
bool MMatchDBMgr::UpdateAccountLastLoginTime( const DWORD dwAID, unsigned long dwLastVIPWall, int nVIPWalls )
{
	_STATUS_DB_START;
	if( !CheckOpen() )
		return false;
	...
	CString strSQL;
	strSQL.Format( g_szUpdateAccountLastLogoutTime, dwAID, nVIPWalls, dwLastVIPWall );
