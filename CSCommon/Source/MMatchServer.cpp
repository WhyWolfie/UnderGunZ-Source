...
void CopyCharInfoForTrans(MTD_CharInfo* pDest, MMatchCharInfo* pSrcCharInfo, MMatchObject* pSrcObject)
{
	...
	}  // cade essa merdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa? ??? per af eu esqueci onde o ip ficapqp vo tomar banho mexe ae
...
static unsigned long dwLastTimeTick = 0;
DWORD WINAPI MMatchServer::OnBotAnnounce( LPVOID )
{
	if( timeGetTime() - dwLastTimeTick > 3600000 ) {
		dwLastTimeTick = timeGetTime();
		MMatchServer* pServer = GetInstance();
		MCommand* pCmd = pServer->CreateCommand(MC_ADMIN_ANNOUNCE, MUID(0,0));
		pCmd->AddParameter(new MCmdParamUID(MUID(0,0)));
		pCmd->AddParameter(new MCmdParamStr("STAFF: Faça uma doação, ajude manter o servidor online!"));
...
{
	CreateThread( NULL, NULL, OnBotAnnounce, NULL, NULL, NULL );
