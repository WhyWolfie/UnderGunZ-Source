...
// custom commands.
void ChatCmd_Comandos(const char* line, const int argc, char **const argv);
...
////////////////////////////////////////////////////////////////////
	...
	//custom commands.
	_CC_AC("comandos",			&ChatCmd_Comandos,				CCF_ALL, ARGVNoMin, 1 , true,"/comandos", "");
	...
	//VIP LIXO
    _CC_AC("vip_wall",		    &ChatCmd_VIPAnnounce,			CCF_ALL, ARGVNoMin, 1 , true,"/vip_wall <¸Þ½ÃÁö>", "");
...
void ChatCmd_Comandos(const char* line, const int argc, char **const argv){
	ZChatOutput("/damage - Ativa do damage log durante o jogo.", ZChat::CMT_SYSTEM);
	ZChatOutput("/nomassive - Ativa ou desativa massives.", ZChat::CMT_SYSTEM);
	ZChatOutput("/fps - [número] - Escolha sua taxa de FPS durante o jogo.", ZChat::CMT_SYSTEM);
	ZChatOutput("/clearchat - Limpa o chat.", ZChat::CMT_SYSTEM);
	ZChatOutput("/swordtrail - Ativa/Desativa rastro da espada.", ZChat::CMT_SYSTEM);
	ZChatOutput("/healtbar - Mostra sua barra de HP/AP.", ZChat::CMT_SYSTEM);
	ZChatOutput("/autorec - Ativa automaticamente a gravação de um video, em .avi (FRAPS).", ZChat::CMT_SYSTEM);
	ZChatOutput("/time - Horário atual do seu computador.", ZChat::CMT_SYSTEM);
	ZChatOutput("/sound - Som 3D.", ZChat::CMT_SYSTEM);
	ZChatOutput("/snipercrosshair - Ativar, Desativar mira da sniper.", ZChat::CMT_SYSTEM);
	ZChatOutput("/camera - Use <150 a 1000>", ZChat::CMT_SYSTEM);
	ZChatOutput("/exit - Sair do jogo.", ZChat::CMT_SYSTEM);
	ZChatOutput("CTRL + S - Mostra seu HP/AP.", ZChat::CMT_SYSTEM);
	ZChatOutput("CTRL + F - Mostra seu FPS.", ZChat::CMT_SYSTEM);
}
void ChatCmd_Help(const char* line, const int argc, char **const argv)
{
	...
	ZChatOutput("CTRL + S - HP/ Mostra seu HP/AP output", ZChat::CMT_SYSTEM);
	ZChatOutput("CTRL + F - Seu FPS", ZChat::CMT_SYSTEM);
	ZChatOutput("--------------------------------", ZChat::CMT_SYSTEM);
	ZChatOutput("Room tag mods:", ZChat::CMT_SYSTEM);
	//ZChatOutput("/nomassive - Desativa o massive", ZChat::CMT_SYSTEM);
	ZChatOutput("--------------------------------", ZChat::CMT_SYSTEM);
...
void ChatCmd_Damage(const char* line, const int argc, char **const argv)
{
	...
	{
		ZChatOutput("^2Damage Counter desativado.", ZChat::CMT_SYSTEM);
		ZGetConfiguration()->GetExtra()->bDamageCounter = false;
	}
	else
	{
		ZGetConfiguration()->GetExtra()->bDamageCounter = true;
		ZChatOutput("^2Damage Counter ativado.", ZChat::CMT_SYSTEM);
	}
}
void ChatCmd_FPS(const char* line, const int argc, char **const argv)
{
	if (argc < 2) 
	{
		ZChatOutput("Insira um valor de FPS!", ZChat::CMT_SYSTEM);
		return;
	}

	RSetFrameLimitPerSeceond(atoi(argv[1]));
	ZChatOutput("FPS mudado com sucesso!", ZChat::CMT_SYSTEM);
}
void ChatCmd_DisableSpecial(const char* line, const int argc, char **const argv)
{	
...
{	
	...
	if(ZGetConfiguration()->GetExtra()->bnomassive)
		ZChatOutput("^2Nomassive ligado.");
	else
		ZChatOutput("^2Nomassive desligado.");
	ZGetOptionInterface()->SaveInterfaceOption();
}
...
{	
	...
	if(ZGetConfiguration()->GetExtra()->bhidecrosshair)
		ZChatOutput("^2Crosshair ligado.");
	else
		ZChatOutput("^2Crosshair desligado.");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_Bullet(const char* line, const int argc, char **const argv)
{	
...
{	
	...
	if(ZGetConfiguration()->GetExtra()->bsword)
		ZChatOutput("^2Rastro da espada ativado ^1(Na próxima sala que você entrar, o rastro estará desativado).");
	else
		ZChatOutput("^2Rastro da espada desativado ^1(Na próxima sala que você entrar, o rastro estará desativado).");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_HealtBar(const char* line, const int argc, char **const argv)
{	
	...
	if(ZGetConfiguration()->GetExtra()->bhpap)
		ZChatOutput("^2Healt bar ativado.");
	else
		ZChatOutput("^2Healt bar desativado.");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_Visible(const char* line, const int argc, char **const argv)
{	
...
{	
	...
	if(ZGetConfiguration()->GetExtra()->bscops)
		ZChatOutput("^2Mira da sniper ativada ^1(Na próxima sala que você entrar, a mira estará ativada.).");
	else
		ZChatOutput("^2Mira da sniper desativada ^1(Na próxima sala que você entrar, a mira estará desativada.).");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_Nosmoke(const char* line, const int argc, char **const argv)
{	
	...
	if(ZGetConfiguration()->GetExtra()->bnosmoke)
		ZChatOutput("^2Rocket smoke ativado.");
	else
		ZChatOutput("^2Rocket smoke desativado.");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_Damagebar(const char* line, const int argc, char **const argv)
{	
	...
	if(ZGetConfiguration()->GetExtra()->bdamage)
		ZChatOutput("^2Damage bar ativado.");
	else
		ZChatOutput("^2Damage bar desativado.");
	ZGetOptionInterface()->SaveInterfaceOption();
}
void ChatCmd_Timebar(const char* line, const int argc, char **const argv)
{	
...
void ChatCmd_Staffcommands(const char* line, const int argc, char **const argv) {
	...
	ZChatOutput("--------------------------------", ZChat::CMT_SYSTEM);
	ZChatOutput("/teleport - Teleporte até o negro", ZChat::CMT_SYSTEM);
	ZChatOutput("/teleportplayer - Teleporte o negro até você", ZChat::CMT_SYSTEM);
	ZChatOutput("/freeze_player - Trave o negro", ZChat::CMT_SYSTEM);
	ZChatOutput("/unfreeze_player - Destrave o negro", ZChat::CMT_SYSTEM);
	ZChatOutput("/shotbotcheck - Checka se o filho da puta tá de shotbot", ZChat::CMT_SYSTEM);
	ZChatOutput("/admin_senditem charname esse n funfa ok flw", ZChat::CMT_SYSTEM);
}
void ChatCmd_iscleytonLinda(const char* line, const int argc, char **const argv) {
	ZChatOutput("Comandos emplementados:", ZChat::CMT_SYSTEM);
	ZChatOutput("/fps - Mostra seu FPS no chat", ZChat::CMT_SYSTEM);
	ZChatOutput("/nomassive - Desliga ou liga massives", ZChat::CMT_SYSTEM);
...
void ChatCmd_Camera(const char* line, const int argc, char **const argv)
{
	...
	if(!ZGetGame())
	{
		ZChatOutput("Use dentro da sala!", ZChat::CMT_SYSTEM);
		return;
	}
	...
	else
	{
		...
		{
			ZChatOutput("Use 150 a 1000!", ZChat::CMT_SYSTEM);
			return;
		}
	}
	...
	sprintf(szMsg, "Camera alterada para %i!", (int)fDist);
