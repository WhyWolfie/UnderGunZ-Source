...
{
sprintf(szName, "!^9Meu HP: %d, AP: %d",(int)ZGetGame()->m_pMyCharacter->GetHP(),(int)ZGetGame()->m_pMyCharacter->GetAP());
...
//MY FPS
sprintf(szName, "^9Meu FPS Atual é: %3.3f",g_fFPS,fScore,fMs);
ZApplication::GetGameInterface()->GetChat()->Input(szName);
}
	...
	if( !hThread || dwCode != STILL_ACTIVE ){
...
bool GetUserGradeIDColor(MMatchUserGradeID gid,MCOLOR& UserNameColor,char* sp_name)
{
	...
	{ 
		...
		UserNameColor = MCOLOR(0,255,255); 
		return true; 
	}
	else if(gid == MMUG_STAR) {
		UserNameColor = MCOLOR(46,139,87); 
		return true; 
	}
...
//VIP
		...
		UserNameColor = MCOLOR(220,20,60); 
		return true; 
	}
	else if(gid == MMUG_VIP2) {
		UserNameColor = MCOLOR(65,105,225); 
		return true; 
	}
	...
	case MC_GUNZ_ANTILEAD:
		{
			...
			for (int i = 0; i < nSize; ++i)
			{
				...
				if(pCommand->GetSenderUID().Low == ZGetGame()->m_pMyCharacter->GetUID().Low)
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageDealt += pInfo->fDamage;
				else
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageTaken += pInfo->fDamage;
			...
			if(pChar) 
			{
				...
				if ( (nTeam == MMT_ALL) || (nTeam == MMT_SPECTATOR))
				{
					if ( !ZGetGameClient()->GetRejectNormalChat() || ( strcmp( pChar->GetUserName(), ZGetMyInfo()->GetCharName()) == 0))
					{
						...
						if(bSpUser) {
							sprintf(szTemp, "%s%s : %s", ((pChar->IsDie()) ? "[MORTO] " : ""), pChar->GetProperty()->GetName(),szMsg);
				...
				else if (nTeam == nMyTeam)
				{
						 ...
						 ( strcmp( pChar->GetUserName(), ZGetMyInfo()->GetCharName()) == 0))
					{
						...
						if(bSpUser) {
							sprintf(szTemp, "%s(Team)%s : %s", ((pChar->IsDie()) ? "[MORTO] " : ""), pChar->GetProperty()->GetName(),szMsg);
							ZChatOutput(UserNameColor, szTemp);
						}
						else {
							sprintf(szTemp, "%s(Team)%s : %s", ((pChar->IsDie()) ? "[MORTO] " : ""), pChar->GetProperty()->GetName(),szMsg);
...
#define SHOTGUN_DIFFUSE_RANGE	0.07f
...
void ZGame::OnPeerReload(MUID& uid)
{
	...
	//Room Tag - [bwzf] instant reload
if(strstr(ZGetGameClient()->GetStageName(),"[bwzf]"))
{
if(pCharacter==m_pMyCharacter)
...
//				ZGetGameInterface()->GetBandiCapturer()->ToggleStart();
			...
			{					
				...
				{
					char szMsg[256] = "";
					int nTeam = ZGetGame()->m_pMyCharacter->GetTeamID();
					sprintf(szMsg, "Eu tirei %i damage e recebi %i damage.", ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageDealt, ZGetGame()->m_pMyCharacter->GetStatus().Ref().nDamageTaken);
