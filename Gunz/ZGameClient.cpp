...
//	gid = MMUG_DEVELOPER;
	...
	if(bSpUser)	// 특수유저
	{
        ...
        {
        case 255 :
        wsprintf(szText, "[Staff] %s : %s", szName , szChat);
            break; 
        case 254:
        wsprintf(szText, "[Staff] %s : %s", szName , szChat);
            break;
        ...
        wsprintf(szText, "[Staff] %s : %s", szName , szChat);
            break;
		case 2:
        wsprintf(szText, "[Event]%s : %s", szName , szChat);
            break;
        case 3:
...
//	gid = MMUG_DEVELOPER;
	...
	if(bSpUser)	// 특수유저
	{
        ...
        {
        case 255 :
        wsprintf(szText, "[Staff] %s : %s", pObjCache->GetName() , szChat);
            break; 
        case 254:
        wsprintf(szText, "[Staff] %s : %s", pObjCache->GetName() , szChat);
            break;
        ...
        wsprintf(szText, "[Staff] %s : %s", pObjCache->GetName() , szChat);
            break;
        case 2:
        wsprintf(szText, "[Event]%s : %s", pObjCache->GetName() , szChat);
            break;
        case 3:
...
void ZGameClient::GetEncryptMD5HashValue2(char* szEncryptMD5Value)							// update sgk 0724
{
	char filePath[MAX_PATH] = {0, };
	GetModuleFileName(NULL, filePath, MAX_PATH);
...
void ZGameClient::OnAdminAnnounce(const char* szMsg, const ZAdminAnnounceType nType)
{
	...
	case ZAAT_VIP:
		{
			ZTransMsg( szText, MSG_VIP_ANNOUNCE, 1, szMsg );
