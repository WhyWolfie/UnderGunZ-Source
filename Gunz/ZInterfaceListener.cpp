...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true){
			...
			{
				ZApplication::GetGameInterface()->ShowMessage("Você não pode criar sala no canal de eventos.");
...
*/
	...
	sprintf(szBuff2, "Reporter Name: %s, Reported Name: %s, Reason: %s.", ZGetMyInfo()->GetCharName(), pCombo1->GetSelItemString(), pCombo2->GetSelItemString());

	ZGetGameInterface()->GetChat()->Report112( szBuff2);
